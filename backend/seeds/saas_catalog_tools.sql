...
INSERT INTO evidence.sources (external_key, source_type, title, publisher, source_url, is_fictional, review_status, reviewed_at, review_note) VALUES
 ...
 ('canva-pricing-2026-08','manufacturer_documentation','Canva pricing','Canva','https://www.canva.com/pricing/',false,'verified',now(),'Read 2026-08-21 through a US connection, after forcing a US path because the default redirect priced the page in Bulgarian lev. Canva publishes only 180 US$ per year for one person on the Pro tier and offers no monthly view; the monthly figure recorded is that divided by twelve, and is a derivation rather than a reading. Source URL corrected 2026-08-28: the en_us path Canva served in August now returns 404, and the canonical pricing page is /pricing/. The price was not re-read, so the observation still carries its 2026-08-21 date.'),
